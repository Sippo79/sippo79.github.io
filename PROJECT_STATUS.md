...
- **構成の参考価格**: **`shared/parts/build-price.js`** + **`shared/parts/part-prices.json`**（2026-09-04〜）
  - PC BUILD CHECK の75構成に「参考価格」を出す仕組み。**計算の実装はこの1ファイルだけ**。
    診断画面（`script.js`）・静的生成（`compute-prices.js` 経由で `generate-builds.ps1`）・
    テストの3者が同じ関数を呼ぶ。⚠️ **計算式を他所へコピペしないこと**（金額が食い違う）。
  - **基準は「同等構成のBTO完成品の実売目安」**（`_meta.basis: "bto"`）。自作パーツの合計ではない。
    パーツ個別合計で計算すると、CPU・GPU以外だけで約71,000円かかり
    「10万円・新品パーツ」が数字上成立しないため（最安の新品GPUがRTX 5050の35,000円）。
    サイトがBTOメーカーへ誘導している実態にも合わせている。
  - **GPU価格は `gpu-guide/gpus.json` が唯一の情報源**。`part-prices.json` に書かない
    （二重管理を `test-build-price.js` が検出する）。価格マスターが持つのは
    CPU7種・メモリ・ストレージ・マザボ・BTO上乗せ（ケース/電源/冷却/OS/組立=25,000円）だけ。
  - **予算超過は隠さず明示する**。予算内に収めるためのパーツ削減はしていない
    （「10万円×4K」のように予算内で成立しない条件が実在し、削っても収まらず性能だけ落ちるため）。
    超過の判定は `OVER_TOLERANCE`（+15%）の1か所。
  - 表示は「約〇万円」の粒度＋変動注記を必ず添える。1円単位で出すと固定価格に見えるため。
    価格が1つでも欠けたら参考価格を出さない（欠けたまま合計すると必ず安く見える）。
...
- **⚠️ 守るべきルール**:
  ...
  - **価格をHTMLに固定表示しない**（古い価格が残る事故を防ぐ）。
    ※これは**アフィリエイト商品の価格**の話。PC BUILD CHECK の「構成の参考価格」
    （`shared/parts/build-price.js`）は別枠で、購入リンクに価格を並べるものではなく、
    構成が選んだ予算に収まるかを示す目安。変動注記を必ず添え、更新日を持つ。
...
### ✅ PC BUILD CHECK は「完成」扱い（2026-09-04 決定）

Phase 1〜8 で、診断精度・中古GPU表示・解像度基準・構成品質・参考価格まで一通り整えた。
**以降は機能追加を続けず、Search Console・クリック計測・実ユーザー行動を見て
必要な改善だけを行う。** 思いつきで機能を足さないこと。

見るべきもの:
- Search Console のインデックス状況・検索クエリ・CTR（どの構成ページが実際に見られているか）
- アフィリエイトのクリック計上（どの導線が機能しているか）
- `part-prices.json` の鮮度（相場が動いたら `updated` と値を見直して静的ページを再生成）

**やらないと決めていること**（蒸し返さない）:
- PC BUILD CHECK への推奨アルゴリズム導入（詳細判断は `/upgrade/` の役割）
- 現在GPU入力・fps入力・候補ランキング・動的required計算の追加
- リアルタイム価格API・スクレイピング（価格は手動更新のマスターで持つ）
- 予算選択肢そのものの変更（URL・構成ID・sitemap に波及するため）
...
- **`gpu-guide/gpus.json` のGPU価格に更新日・出典のフィールドが無い**。表示は「価格目安」
  「時期によって変動」と明記しているので誤認は招かないが、古さの判断材料が無い。
  ※構成の参考価格マスター（`shared/parts/part-prices.json`）は 2026-09-04 に
  `_meta.updated` と各エントリの `updated` を持たせ、テストで必須にした。
  `gpus.json` 側にも同様のフィールドを足すかは検討の余地あり（自動取得基盤は作らない方針）。
...
### 解消済み（参考）
- ~~「20万円構成」と表示しているのに実際のパーツ合計が大きく20万円を超えていた（参考価格の表示が無く、予算と実額の食い違いを利用者が判断できなかった）~~ → **2026-09-04 解消**（Phase 8 / BTO完成品基準の参考価格を診断結果と静的75ページに表示。乖離の中央値 +43% → +13.5%。予算内で成立しない条件は超過を明示する方針とし、構成のパーツ削減はしていない）。
