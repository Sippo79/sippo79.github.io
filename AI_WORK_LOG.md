...
- **変更ファイル**: `gpu-guide/common.css`, `style.css`, `gpu-guide/sw.js`, `sw.js`
- **変更内容**:
  - **【配色】GPU GUIDE のヘッダーボタンにサイトのアクセント色（シアン `#19e3ff`）を適用**。文字 `#8eeafa` ／ 枠 `rgba(25,227,255,.34)` ／ 背景 `rgba(25,227,255,.10)` ＋ `font-weight:700`。ホバー時はブランドグラデーションを敷いて反転させる。**文字とヘッダー背景のコントラスト比は 14.30:1（WCAG AA 合格）** を実測で確認。
  - **【ドロワーのずれ】原因は「以前の修正が戻った」のではなく、"最初から効いていなかった"**。
    - `style.css` の `.nav__svcnav .sippo-svcnav { width: 100% }` は **git履歴上 `2b0ff7e` で入って以来一度も変わっておらず**、退行ではなかった。
    - 実際の原因は、`.nav__svcnav`（`display:flex`）と `.sippo-svcnav` の**間に、sippo-nav.js が差し込むマウント先 `<div data-sippo-servicenav>` が挟まっている**こと。この div は `sippo-nav.css` で `inline-flex`（＝中身幅）のため、**中の要素にいくら `width:100%` を指定しても、この div 自体が約107pxに縮んで左に寄る**。他のメニュー項目は中央揃えのため、ここだけ左にずれて見えていた。
    - **マウント先の div にも幅指定を追加**して解決（`display:flex` / `width:100%` / `flex:1 1 100%`）。実測で **342px幅・中央 195px** となり、他項目（すべて中央195px）と完全に揃った。320 / 390 / 500 / 820px で確認済み。
  - Service Worker を版上げ（`sippo-pc-v10 → v11` / `gpu-guide-v8 → v9`）。
- **影響範囲**: CSSのみ。HTML・JSは未変更。既存テスト **598件すべて成功**。全195ページを 320 / 390px で再監査し **横スクロール0件 / 重なり0件**。
- **未対応・次にやること**: 特になし。
- **別AIへの引き継ぎ注意点**:
  - ⚠️ **共通サービスナビは「マウント先の `<div data-sippo-servicenav>`」が必ず1階層挟まる**。位置やサイズがどうしても効かないときは、`.sippo-svcnav` ではなく**この div を疑うこと**（`sippo-nav.css` で `inline-flex` 指定）。今回の「何度直しても戻る」の正体はこれ。
  - ⚠️ flex の子に `width: 100%` を指定しても、`flex-basis:auto` / 中身幅の親が挟まると効かない。階層すべてを伸ばす必要がある。

---

## 2026-09-17 — GPU GUIDE のスマホヘッダーが縦書き化していた問題を修正

- **修正目的**: 「GPU GUIDE のヘッダー表示が気になる。ランキングなどのボタンに枠を付けるか、ボタン数を減らしてランキングと比較表のみでもよい」への対応。
- **変更ファイル**: `gpu-guide/common.css`, `gpu-guide/sw.js`
- **変更内容**:
  - **原因**: 520px以下で `.header-link` が `padding: 0 8px` かつ **`white-space` 未指定（= normal）** だったため、ロゴ＋サービスナビ＋リンク4つ（ランキング/比較表/おすすめ/選び方）を1行に詰めた結果、**日本語が1文字ずつ折り返って縦書きのようになっていた**。実測で各リンクは**幅35〜48px・高さ54px**まで潰れ、ヘッダー（61px）からもはみ出していた。
  - **スマホでは主要な2つ（ランキング / 比較表）だけ残し、枠付きの丸ボタンにした**（ユーザー選択）。`white-space: nowrap` を付けて折り返しを止め、`border` + 薄い背景で他サイトのヘッダーと同じ見た目に統一。実測で **幅89px / 64px・高さ44px の1行**に収まった。
  - 「おすすめ」「選び方」はスマホでのみ非表示。**「おすすめ」はヒーローの『用途別おすすめを見る』ボタンから到達できる**。「選び方」はページを下にたどれば到達できる（セクション自体は残っている）。
  - ⚠️ **GPU個別ページ65件は相対パス**（`../../index.html#recommend`）でリンクしているため、属性セレクタは**末尾一致 `[href$="#recommend"]`** にした。前方一致（`[href="#recommend"]`）だと個別ページに効かず、320px で「選び方」が画面外にはみ出したままだった（修正済み）。
  - **PC表示（521px以上）は従来どおり4つとも表示・枠なし**で変更していない。
  - Service Worker を版上げ（`gpu-guide-v7 → v8`）。`common.css` はキャッシュ対象のため。
- **影響範囲**: `gpu-guide/common.css` のスマホ用指定のみ。HTMLは未変更（リンクは残したままCSSで出し分け）。既存テスト **598件すべて成功**。全195ページを 320 / 390px で再監査し **横スクロール0件 / 重なり0件**。
- **未対応・次にやること**: 特になし。
- **別AIへの引き継ぎ注意点**:
  - ⚠️ ヘッダー内の日本語リンクには **`white-space: nowrap` を必ず付ける**。付けないと幅が足りないときに1文字ずつ折り返して縦書き化する（同じ事故が game-pc-guide でも過去に発生している）。
  - ⚠️ GPU GUIDE のヘッダーリンクは**トップと個別ページでhrefの書き方が違う**（絶対 `#compare` / 相対 `../../index.html#compare`）。CSSで出し分けるときは末尾一致セレクタを使うこと。

---

## 2026-09-17 — PC構成投稿サイトのヘッダー追従化／共通ナビのボタン表示を変更

- **修正目的**: 「PC BUILDS HUB でカテゴリから探す〜新規登録までのボタンの配置が揃っていない」「サービスボタンのプルダウンを開いたまま下へスクロールするとボタンが追従せず扱いづらい」「ヘッダーを他ページ同様スクロールしても表示されるようにしたい」「『サービス』ではなく分かりやすい言い回しにしたい」への対応。
- **変更ファイル**: `pc-builds-hub/style.css`, `shared/nav/sippo-nav.js`, `shared/nav/sippo-nav.css`, `game-pc-guide/style.css`, `sw.js`, `game-pc-guide/sw.js`, `gpu-guide/sw.js`, `pc-build-check/sw.js`, `pc-builds-hub/sw.js`
- **変更内容**:
  - **【ヘッダー追従】`.top-header` を sticky 化した**。pc-builds-hub は post.html の `.site-header` だけが sticky で、それ以外の**10ページが使う `.top-header` は sticky ではなかった**ため、下へスクロールするとヘッダーごと流れて消えていた（プルダウンを開いたままスクロールするとボタンが画面外に行く、という報告の原因）。
    - ⚠️ **ただし sticky を指定しただけでは効かなかった**。`.index-page { overflow-x: hidden }` があると **body 自身がスクロール枠**になり、中の sticky が「画面の上」ではなく body 基準になって貼り付かない。`overflow-x: clip` に変更して解決（clip は横はみ出しを抑えつつ sticky を壊さない。この方針はリポジトリ内で既に採用済み）。
    - 背景は半透明（0.94）だと下の文字が透けて読みにくかったため**不透明** `#050709` に。`margin-inline: calc(50% - 50vw)` で画面幅いっぱいに背景を伸ばしつつ、中身の位置は動かさない。
  - **【ボタンの不揃い】ヘッダー内ボタンの見た目を統一した**。「カテゴリから探す」「構成一覧」「ログイン」「新規登録」は `.btn`（16px / weight 400 / padding 8px 18px）なのに、サービスナビのボタンだけ共通CSS側の指定（**14px / weight 700 / padding 9px 14px**）が効いており、**文字サイズも太さも余白も揃っていなかった**。同じ行に並ぶボタンとして基準を合わせ、スマホでは5つとも `min-height: 40px` / `font-size: .85rem` に統一。
  - **【スマホで高さを取りすぎない】** sticky 化でヘッダーが常時見えるため、スマホでは余白とロゴを詰めた（**177px → 144px**、画面の約17%）。
  - **【ボタンの言い回し】「サービス」→ 現在のサイト名（正式名から「PC」を抜いた短縮名）に変更**。`SERVICES` 配列に `short` を追加し、`PC BUILD CHECK → BUILD CHECK` / `GAME PC GUIDE → GAME GUIDE` / `PC UPGRADE → UPGRADE` / `GPU GUIDE` / `PC構成投稿サイト → 構成投稿` / `シッポPC相談室 → 相談室` / `シッポPC → シッポTOP` とした。
    - 補足説明「ほかのSippoサイト」は**画面が広いときだけ**ボタン内に併記し（例: 「BUILD CHECK｜ほかのSippoサイト」）、スマホでは隠してサイト名だけにする。
    - ⚠️ 検討過程で一度ボタン文言を「ほかのSippoサイト」固定にしたが、**game-pc-guide のヘッダーが 107px → 156px に肥大化**し（4列グリッドに収まらず3行になる）、**ロゴとボタンが62px重なる**不具合も出たため取りやめた。短縮サイト名方式なら従来どおり107pxに収まることを実測で確認済み。
  - **Service Worker の版上げ**: `style.css` / `shared/nav/*` を変更したため5本すべて更新（`sippo-pc-v9→v10` / `game-pc-guide v4→v5` / `gpu-guide-v6→v7` / `pc-build-check-v8→v9` / `pc-builds-cache-v11→v12`）。
- **影響範囲**: CSSと共通ナビJSのみ。HTMLの構造・Supabase連携（認証/投稿/Nice/管理者）には触れていない。既存テストは **598件すべて成功**。全195ページを 320 / 390px で再監査し、**横スクロール0件 / 要素の重なり0件**。
- **未対応・次にやること**:
  - ボタン表示は**サイト名のみ**とする方針（ユーザー判断）。スマホでは「今いる場所の表示」に見え、押すと他サイトへ行けることが伝わりにくい可能性は残るため、迷う様子があれば「他サイト：○○」形式への変更を再検討する。
- **別AIへの引き継ぎ注意点**:
  - ⚠️ **`overflow-x: hidden` を親要素に付けると position: sticky が効かなくなる**。横はみ出し対策には `overflow-x: clip` を使うこと。
  - ⚠️ 共通ナビのボタン文言を長くすると **game-pc-guide（520px以下の4列グリッド）が破綻する**。文言を変える前に同ページのヘッダー高さと重なりを必ず実測すること。
  - ⚠️ サイトを追加したら `SERVICES` に `short`（正式名から「PC」を抜いた短縮名）も忘れず入れる。未指定なら `name` にフォールバックする。

---

## 2026-09-17 — 全ページのスマホ表示を監査し、ボタンのずれ・重なりを修正

- **修正目的**: スマホで見るとページによってボタン配置がずれる／重なるという報告への対応。全196ページ（実測195ページ＋offline.html）を実機幅で再確認し、原因を特定して直す。
- **変更ファイル**: `shared/nav/sippo-nav.css`, `shared/nav/sippo-nav.js`, `game-pc-guide/style.css`, `gpu-guide/common.css`, `gpu-guide/style.css`, `pc-build-check/style.css`, `pc-build-check/builds.css`, `pc-consult/style.css`, `upgrade/style.css`, `sw.js`, `game-pc-guide/sw.js`, `gpu-guide/sw.js`, `pc-build-check/sw.js`
- **変更内容**:
  - **【主因】共通サービスナビのパネルがヘッダーに重なっていた**。`sippo-nav.css` のスマホ用指定が縦位置を `top: 56px` と**決め打ち**していたが、実際のヘッダー高さはサイトごとに **48〜107px** とばらついており（pc-build-check 61px / gpu-guide 61px / 親サイト 72px / pc-consult 86px / game-pc-guide 107px）、**6サイトでパネルがヘッダーに重なっていた**（game-pc-guide で最大51px、pc-consult で30px）。`sippo-nav.js` が開くたびにトグルボタンの実座標を測って CSS 変数 `--svcnav-panel-top` に入れ、**ボタンの真下8px**に出すよう変更。resize / scroll でも測り直す。これで全サイトのパネル位置が揃った。
  - **【はみ出し】game-pc-guide のヘッダーでナビボタンが画面外に出ていた**。520px以下で `.header-inner` が4列グリッド、`.header-nav` が `display: contents` のため、サービスナビのマウント先 `<div>` が**配置指定を持たないまま自動で4列目に入り**、ボタン幅（約94px）が1列分（約84px）を超えて **390px幅で右端378px→385px まではみ出していた**。`[data-sippo-servicenav]` に `grid-row: 1 / grid-column: 4 / justify-self: end` を明示して解決。
  - **【ボタンが2行に膨らむ】pc-consult のCTA**。`.btn--lg` の左右40pxパディング＋1.1rem のまま横幅いっぱいに広げていたため、「500円ワンコイン相談を申し込む」が1行に収まらず**語中で折れて約101px高**になっていた。スマホでは余白と文字を詰め（`padding: 16px 20px; font-size: 1rem`）、`word-break: keep-all` を併用。**101px → 55px の1行**に収まり、他のボタンと高さが揃った。
  - **【タップ領域】文字高しかないリンクを押しやすくした**。ヘッダーのロゴ（game-pc-guide は 273×**15px** の薄い帯だった）、パンくず（約17〜20px）、`.back-link`、構成表のGPUリンクに、見た目を変えずに縦の当たり判定を追加（`min-height: 44px` または上下 `padding`）。
  - **Service Worker の版上げ**: `style.css` / `common.css` / `builds.css` / `shared/nav/*` を変更したため、キャッシュ優先配信で旧CSSが配られないよう4本を更新（`sippo-pc-v8→v9` / `game-pc-guide v3→v4` / `gpu-guide-v5→v6` / `pc-build-check-v7→v8`）。
- **影響範囲**: CSSとナビJSのみ。HTMLの構造・文言・リンク先・診断ロジック・Supabase関連には触れていない。既存テストは **598件すべて成功**（診断ロジック145 / 参考価格349 / Service Worker 43 / サイト間導線61）。
- **検証方法**: ヘッドレスChrome（CDP直接操作、追加パッケージのインストールなし）でローカルHTTPサーバー経由に全195ページを読み込み、**320 / 360 / 390 / 414px** の4幅で、①横スクロールの発生 ②操作要素どうしの重なり ③小さすぎるタップ領域 を自動計測した。結果は **横スクロール0件 / 重なり0件**、タップ領域の指摘は **571件 → 22件（約96%減）**。残る22件は本文中のインラインリンクと pc-builds-hub のログイン補助リンクで、いずれも文中に収める必要がある箇所のため意図的に据え置き。
- **未対応・次にやること**:
  - `pc-builds-hub` の認証まわりの補助リンク（`.auth-link` 等、約14件）は44px未満のまま。認証UIの改修時にあわせて検討する。
  - ブレークポイントがサイトごとに **400/420/480/520/560/600/640/720/768/800/820/900/1024px** とばらついている。今回は挙動の不具合のみを直し、統一はしていない（影響範囲が大きいため）。
- **別AIへの引き継ぎ注意点**:
  - ⚠️ **ヘッダー高さを決め打ちした指定を再び入れないこと**。サイトごとに 48〜107px と違う。共通ナビのパネル位置は `sippo-nav.js` の `place()` が実測して `--svcnav-panel-top` に入れる仕組みになっている。
  - ⚠️ `game-pc-guide` の 520px 以下のヘッダーは `display: contents` のグリッド。ここに要素を足すときは **`grid-row` / `grid-column` を明示**しないと自動配置ではみ出す。
  - ⚠️ **`style.css` / `builds.css` / `shared/nav/*` を変更したら、対応する `sw.js` の版を必ず上げること**（キャッシュ優先配信のため、上げないと再訪ユーザーに旧CSSが配られる）。
